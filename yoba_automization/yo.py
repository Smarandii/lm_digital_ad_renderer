...
class Size:
    ...
    def create_preview(self):
        ...
        logger.debug(f"RENAME: {os.path.join(self.DIR_PATH.parent, self.name).__str__()} to the {os.path.join(self.DIR_PATH.parent, self.name_with_preview)}")
        ...
        logger.debug(f"RENAME: {os.path.join(self.DIR_PATH.parent.parent, self.name).__str__()} to the {os.path.join(self.DIR_PATH.parent.parent, self.name_with_preview)}")
    ...
    def render_video(self):
        ...
        subprocess.call(f"ffmpeg -y -loop 1 -i \"{self.jpg_path}\" -t {self.video_duration} -pix_fmt yuv420p \"{self.video_path}\"", shell=True)
        if self.fade_in_fade_out:
            ...
            subprocess.call(f"ffmpeg -y -i \"{self.video_path}\" -vf \"fade=t=in:st=0:d=1\" \"{f_in_fname}\"", shell=True)
            subprocess.call(f"ffmpeg -y -i \"{f_in_fname}\" -vf \"fade=t=out:st=4:d=1\" \"{f_in_f_out_fname}\"", shell=True)
